...
?>
    <?php foreach($letters as $i => $letter):?>
        <?php
            ...
            if($year = $this -> state -> get('filter.year')):
                $date   = '&year='.$year;
            endif;

            if($this -> state -> get('filter.year') && $month = $this -> state -> get('filter.month')):
                $date   .= '&month='.$month;
            endif;
        ?>
        <li>
        <a<?php if($availLetter[$i] != false) echo ' href="'.JRoute::_('index.php?option=com_tz_portfolio&'
            .'view=date'.$date.'&char='.mb_strtolower(trim($letter)).'&Itemid='.JRequest::getInt('Itemid')).'"';?>
