 ...
 */
class TZ_PortfolioModelCategory extends JModelAdmin
{
	/**
...
//		$params = JComponentHelper::getParams('com_categories');
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_tz_portfolio.category' . $extension, 'category', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_tz_portfolio.edit.' . $this->getName() . '.data', array());
		...
		if (JRequest::getVar('task') == 'save2copy')
		{
            ...
            if($groupid){
                ...
                elseif(!empty($post['tz_image_url'])){
                    ...
                    if(isset($post['tz_category_hidden'])){
                        $image  = $this -> uploadImages($post['tz_category_hidden']);
                    }
                }
                ...
                $query  = 'UPDATE #__tz_portfolio_categories SET `groupid`='.$groupid[0]
                          .',`images`="'.$image.'"'
	...
	*/
	...
	protected function batchCopy($value, $pks, $contexts)
	{
		...
		while (!empty($pks) && $count > 0)
		{
            ...
            // Get information with this category
            if(count($oldId)>0){
//                if(count($oldId) > 0)
//                    $currentId  = implode(',',$oldId);
//                if(count($oldParentId) > 0)
//                    $currentId  = implode(',',$oldParentId);

                $val    = null;
                $query2 = 'SELECT * FROM #__tz_portfolio_categories'
                          .' WHERE catid IN('.$oldId.')';
                $db -> setQuery($query2);
                if(!$db -> query()){
                    $this -> setError($db -> getErrorMsg());
                    return false;
                ...
                if(count($rows)>0){
                    foreach($rows as $row){
                        $imageName  = '';
                        if(!empty($row -> images)){
                            $imageName  = $this -> uploadImages($row -> images);
                        }
                        $val[]  = '('.$row -> groupid.','.$newId.',"'.$imageName.'")';
                    }
                }
                // Copy
                if(isset($val) && $val && count($val)>0){
                    $val    = implode(',',$val);
                    $query2 = 'INSERT INTO #__tz_portfolio_categories(`groupid`,`catid`,`images`)'
                              .' VALUES '.$val;
                    $db -> setQuery($query2);
                    if(!$db -> query()){
                        $this -> setError($db -> getErrorMsg());
